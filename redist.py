...
def download(tag: str, *, clean: bool) -> None:
    ...
    with CONSOLE.status(f"Fetching {UPSTREAM} [bold]{tag}[/bold]...") as status, Session() as session:
        ...
        def download_file(dst: Path, url: str) -> None:
            if dst.exists():
                debug(f"skipping cached file download: {dst.relative_to(ROOT_DIR)}")
                return

            status.update(f"Downloading [bold]{dst.relative_to(ROOT_DIR)}[/bold]...")
        ...
        if exe_dir.is_dir():
            debug(f"skipping cached download: {exe_dir.relative_to(ROOT_DIR)}")
            return
        ...
        for asset in release["assets"]:
            exe_name = asset["name"]
            if not asset_filter.match(exe_name):
                debug(f"Ignoring asset: {exe_name}")
                continue

            exe_file = exe_dir / exe_name
            if exe_file.is_file():
                debug(f"skipping cached exe: {exe_file.relative_to(ROOT_DIR)}")
                continue
            download_file(exe_file, asset["browser_download_url"])
...
def build(*, clean: bool, tag_glob: str) -> None:
    ...
    for src in EXE_CACHE_DIR.glob(tag_glob):
        ...
        for exe_src in src.glob(f"{EXE_NAME}*"):
            ...
            shutil.copyfile(exe_src, dst / exe_platform)
